...
class ReportsViewModel(private val repo: ReportRepo?): BaseViewModel() {
    ...
    fun setReportRequest(sDate: String?, eDate: String?, type: String?) {
        hMap.clear()
        sDate?.let{
            hMap[AppConstants.RequestParam.from]= getDifferentInfoFromDate_String(it, AppConstants.DateFormat.DATE_FORMAT_8, DATE_FORMAT_10)
        }
        eDate?.let{
            hMap[AppConstants.RequestParam.to]= getDifferentInfoFromDate_String(it, AppConstants.DateFormat.DATE_FORMAT_8, DATE_FORMAT_10)
        }
        type?.let{
